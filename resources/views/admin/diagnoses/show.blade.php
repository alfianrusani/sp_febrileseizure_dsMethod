...
<div class="row mt-3 g-3">
  ...
  <div class="col-lg-4">
    <div class="card">
      <div class="card-header"><h3 class="card-title"><i class="bi bi-person-badge me-2 text-primary"></i>Data Pasien</h3></div>
      <div class="card-body">
        <div class="d-flex align-items-center gap-3 mb-4">
          <div style="width:60px;height:60px;background:var(--tblr-primary);color:#fff;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:24px;font-weight:700">
        ...
        <table class="table table-borderless table-sm mb-0">
          ...
          <tr>
            ...
            <td>{{ $diagnosis->age_months }} bulan ({{ floor($diagnosis->age_months / 12) }} thn {{ $diagnosis->age_months % 12 }} bln)</td>
          </tr>
          @if($diagnosis->phone)
          <tr>
    ...
    {{-- DS Values --}}
    <div class="card mt-3">
      <div class="card-header"><h3 class="card-title"><i class="bi bi-cpu me-2 text-primary"></i>Nilai Dempster-Shafer</h3></div>
      <div class="card-body">
        <div class="mb-3">
          <div class="d-flex justify-content-between mb-1">
            <small class="text-secondary">Belief — Bel(X)</small>
            <small class="font-weight-medium text-primary">{{ round($diagnosis->belief_value * 100, 2) }}%</small>
          </div>
          <div class="progress" style="height:8px;border-radius:50px">
            <div class="progress-bar bg-primary" style="width:{{ round($diagnosis->belief_value * 100) }}%;border-radius:50px"></div>
          </div>
        </div>
        <div class="mb-3">
          <div class="d-flex justify-content-between mb-1">
          ...
          <div class="progress" style="height:8px;border-radius:50px">
            <div class="progress-bar bg-orange" style="width:{{ round((1 - $diagnosis->belief_value) * 100) }}%;border-radius:50px"></div>
          </div>
        </div>
        <div class="row g-2 mt-1">
          <div class="col-6">
            <div class="p-2 rounded text-center" style="background:#e8f1fd">
              <div style="font-size:20px;font-weight:700;color:var(--tblr-primary)">{{ $diagnosis->belief_value }}</div>
              <small class="text-secondary">Bel(X)</small>
            </div>
          </div>
          <div class="col-6">
            <div class="p-2 rounded text-center" style="background:#fef0e7">
              <div style="font-size:20px;font-weight:700;color:#f39c12">{{ $selectedSymptoms->count() }}</div>
  ...
  <div class="col-lg-8">
    ...
    <div class="card" style="overflow:hidden">
      <div style="background:linear-gradient(135deg,{{ $diagnosis->disease->code === 'P1' ? '#106eea,#0d58c0' : '#e84545,#c0392b' }});padding:24px;color:#fff">
        <div class="d-flex align-items-center gap-3">
          ...
          <div class="flex-fill">
            <div style="opacity:.8;font-size:12px;text-transform:uppercase;letter-spacing:1px">Hasil Diagnosa</div>
      ...
      <div class="card-body">
        <p class="text-secondary" style="font-size:14px;line-height:1.8">{{ $diagnosis->disease->description }}</p>
        @if($diagnosis->disease->treatment)
        <hr>
        <h6 class="font-weight-medium mb-2"><i class="bi bi-heart-pulse text-danger me-2"></i>Rekomendasi Penanganan</h6>
        <div style="font-size:13px;color:#555;line-height:1.9;white-space:pre-line">{{ $diagnosis->disease->treatment }}</div>
        @endif
      </div>
    </div>
    @else
    <div class="card">
      <div class="card-body text-center py-5">
        ...
        <p class="text-secondary">Gejala tidak cukup untuk diagnosis. Diperlukan konsultasi dokter langsung.</p>
      </div>
    </div>
    @endif
    ...
    <div class="card mt-3">
      ...
      <div class="card-body">
        <div class="row g-2">
          ...
          <div class="col-md-6">
            <div class="d-flex align-items-center gap-2 p-2 rounded" style="background:#e8f1fd;font-size:13px">
              <i class="bi bi-check-circle-fill text-primary"></i>
              <div>
                <div class="font-weight-medium">{{ $symptom->name }}</div>
                <div class="d-flex gap-2">
                  <span class="badge bg-secondary text-white" style="font-size:10px">{{ $symptom->code }}</span>

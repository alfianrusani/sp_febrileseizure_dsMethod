...
<head>
    ...
    <style>
        ...
        .result-box.danger h5 {
            color: #dc3545 !important;
        }

        .result-box.danger .percentage {
            color: #dc3545 !important;
        }
...
<body>
    ...
    <div class="container" style="max-width:700px">
        ...
        <div class="mb-4">
            ...
            <table class="w-100">
                <tr>
                    <td width="30%" class="text-muted">Nama Pasien</td>
                    <td>: <strong>{{ $diagnosis->patient_name }}</strong></td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td>: {{ $diagnosis->birth_date->format('d F Y') }}</td>
                </tr>
                <tr>
                ...
                @endif
                <tr>
                    <td class="text-muted">Tanggal Diagnosa</td>
                    <td>: {{ $diagnosis->diagnosis_date->format('d F Y') }}</td>
                </tr>
            </table>
        </div>
        ...
        <div class="mb-4">
            ...
            <ol style="padding-left:18px">
                @foreach ($selectedSymptoms as $symptom)
                    <li>{{ $symptom->name }} <small class="text-muted">({{ $symptom->code }}, densitas:
                            {{ $symptom->density }})</small></li>
                @endforeach
            </ol>
        </div>
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            <div style="font-size:11px;color:#666;text-transform:uppercase;letter-spacing:1px">
                                Didiagnosa dengan</div>
                            <h5 class="mb-0 mt-1"
                                style="color:{{ $diagnosis->disease->code === 'P2' ? '#dc3545' : '#106eea' }};font-weight:700">
                                {{ $diagnosis->disease->name }}</h5>
                            <small class="text-muted">Kode: {{ $diagnosis->disease->code }}</small>
                        </div>
                        <div class="text-center">
                            <div
                                style="font-size:32px;font-weight:800;color:{{ $diagnosis->disease->code === 'P2' ? '#dc3545' : '#106eea' }}">
        ...
        @if ($treatment)
            <div class="mb-4">
                ...
                <div style="font-size:12px;line-height:1.8;white-space:pre-line;color:#444">
                    <strong>Penanganan Pertama:</strong><br>{{ $treatment->first_step_handling }}
                    <br><br>
                    <strong>Obat Penanganan:</strong><br>{{ $treatment->medicine }}
                </div>
            </div>
        @endif
        ...
        <div class="mb-4">
            ...
            <table class="table table-sm table-bordered">
                <tr>
                    <td>Nilai Belief Bel(X)</td>
                    <td><strong>{{ $diagnosis->belief_value }}</strong></td>
                </tr>
                <tr>
                    <td>Nilai Plausibility Pls(X)</td>
                    <td><strong>{{ round(1 - $diagnosis->belief_value, 4) }}</strong></td>
                </tr>
                <tr>
                    <td>Jumlah Gejala Dipilih</td>
                    <td><strong>{{ $selectedSymptoms->count() }}</strong></td>
                </tr>
            </table>
        </div>
